...
def sort(queue, im, path=None, reverse=False, mirror=False, angle=0,
         to_interval=False, max_intervals=0, randomize=False, merge=False,
         low_threshold=0, func=None):
    im = im.convert("RGBA")
    im_bytes = im.tobytes()
    im_bytes_p = ctypes.c_char_p(im_bytes)

    data_ptr = DLL.sortImage(im.width, im.height, im_bytes_p,
                             bytes(path, "UTF-8"), max_intervals, randomize,
                             angle, merge, reverse, mirror, to_interval,
                             low_threshold, bytes(func, "UTF-8"))
